...
> [!NOTE]  
> Этот гайд подходит для роутеров, имеющих версию OpenWRT 24.10.1 или новее, например Xiaomi 4A v2, Xiaomi 4C, Xiaomi AX3000T v1, ASUS RT-AC1200 и др.

<br>

## Настройка авторизации в сети МИЭТ<br>

1. Вставьте в WAN-разъем роутера кабель от провайдера (обычно WAN-разъем имеет другой цвет либо же подписан соответствующе). Если кабель рабочий, то на роутере должен загореться соответствующий светодиод (у разных моделей он находится в разных местах). Подключитесь к роутеру по Wi-Fi или по проводу. Если подключаетесь к роутеру по проводу, не забудьте отключить на вашем компьютере проверку подлинности по стандарту IEEE 802.1x. Если же вы подключаетесь по Wi-Fi, то название сети Wi-Fi по умолчанию указано внизу роутера. Пароль Wi-Fi по умолчанию - 1234567890. После того, как вы подключились к роутеру, откройте браузер и перейдите по адресу 192.168.1.1. Логин по умолчанию - `root`, пароль - `admin`. <br><br>
...
5. Во вкладке MIET нажмите по очереди Enable, а затем Start. Спустя какое-то время в системном журнале появится несколько строчек. Последние строчки должны быть вот такими: <br><br>

> [!NOTE]  
> На некоторых устройствах вместо ```wan``` будет написано ```eth0.2```, как на скриншоте выше. Это тоже правильно.
...
daemon.notice wpa_supplicant[1234]: wan: CTRL-EVENT-EAP-SUCCESS EAP authentication completed successfully
...
> [!WARNING]  
> Если вы подключены к роутеру через Wi-Fi, у вас будет 90 секунд на то, чтобы подключиться к новой сети, иначе настройки не применятся.
